...
serve as heaters. The Qarnot servers also embed several sensors that, in the environment in which they are deployed, could continuously 
...
platform, complex in-situ workflows for smart-homes and buildings could be developed. For this, we consider a typical problem that could be addressed
in such environments: the detection of alarm sounds. Our paper proposes a general model for orchestrating in-situ workflows on 
the Qarnot platform and an implementation for alarm sounds detection. 
We also provide an experimental evaluation of the implemented solution where we discuss of the accuracy, granularity of parallelism and scalability of the training process. The results we obtain are encouraging, they comfort the idea that the Qarnot model is certainly one of the most effective platform for the decentralization of machine learning systems. 
\end{abstract}

\begin{IEEEkeywords}
in-situ machine learning; decentralized machine-learning; alarm sounds detection;  performance evaluation
...
workloads into {\it modest} computing platforms available in homes, offices etc. In this paper, we are interested in building decentralized machine learning 
...
Considering data or processes, the Qarnot model is very rich for in-situ machine learning. Indeed, in the data viewpoint, 
...
The global objective of this paper is to show that with these two tools (QaIoT and the Qarnot SDK), 
complex in-situ machine learning workflows could be developed for smart-homes and buildings. We focus on a problem that is interesting to address with in-situ learning: the construction of alarm sound classifiers.  Indeed, the behavior of an alarm sound classifier will be related to the background noises that 
...
On the construction of machine learning systems with acoustic features, the Auditeur system~\cite{Nirjon:2013:AMS:2462456.2464446} is closely related to ours. The system proposes a context-aware solution for acoustic events detection on smartphones. 
...
the notion of disk and the docker image. A task is an object oriented abstraction that refers to a 
...
\item \textbf{QTask:} QTask inheritates from QWork and Task (from the qarnot SDK). The QTask class has two particularities : 
...
\item \textbf{QCreateDisk:} (inherits from QFunc) This function aims to create a disk with the dataset and all the code needed for selection.
  ...
  \begin{itemize}
  \item The qarnot connection (which will define the building and home where the task will be scheduled)
...
\end{itemize}
Figures \ref{fig:implementation_diagram} and \ref{fig:implementation_code} show the proposed implementation of the framework using the orchestrator. Note that we added a QCreateDisk after QExtractFeaturesTask to add the code needed for selection (this can be avoided by putting the code in the docker repository used for the tasks).
\begin{figure}[H]
\centering
...
\subsection{Features}\label{subsec:extraction}
...
\begin{figure}[h]
  ...
  \end{subfigure}
\end{center}
\caption{Runtimes for coarse-grained implementation \label{fig:granularity}}
\end{figure}
...
\begin{figure}[h]
  ...
  \end{subfigure}
\end{center}
\caption{Runtimes for fine-grained implementation \label{fig:scalability}}
\end{figure}
